...
export default function AccountsPage() {
  ...
  const loadMasterData = async () => {
    ...
    if (industryRes.data) {
      const realEstate = industryRes.data.find((ind: any) => ind.code === 'real_estate');
      if (realEstate) setRealEstateIndustryId(realEstate.id);
    }
  ...
  const loadAccountItems = async () => {
    ...
    } else if (realEstateIndustryId) {
      params.industry_id = realEstateIndustryId;
    }
    ...
    const { data, error } = await accountItemsApi.getAll(params);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingItem) {
      const { error } = await accountItemsApi.update(editingItem.id, itemData);
      ...
      const { error } = await accountItemsApi.create(itemData);
  ...
  const handleToggleActive = async (item: AccountItem) => {
    ...
    const { error } = await accountItemsApi.update(item.id, { is_active: !item.is_active });
