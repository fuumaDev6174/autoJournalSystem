...
export default function UploadOnlyPage() {
  ...
  useEffect(() => {
    ...
    const loadDocs = async () => {
      const { data } = await documentsApi.getAll({ client_id: selectedClientId });
      if (data) {
        // Sort by created_at desc and limit to 20 (backend may not support limit param)
        ...
        setPendingCount(sorted.filter((d: any) => d.ocr_status === 'pending').length);
      }
    };
    loadDocs();
  }, [selectedClientId]);
