...
export default function InvoicePanel() {
  ...
  // OCR抽出データから初期値を取得
  const invoiceNumber = (form as Record<string, unknown>).invoiceNumber as string | undefined
    ?? classification?.extracted_invoice_number as string | undefined ?? '';

  const qualValue = qual === 'qualified' ? '適格請求書（100%控除）'
    : qual === 'kubun_kisai' ? '区分記載（80%控除）' : '';
    ...
    <div className="bg-green-50 border border-green-200 rounded-lg p-3">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div>
          ...
          <input type="text" defaultValue={invoiceNumber} placeholder="T0000000000000"
        ...
        <div>
          ...
          <select defaultValue={qualValue} className="w-full border border-green-300 rounded p-1.5 text-sm bg-white">
            <option value="">--</option>
