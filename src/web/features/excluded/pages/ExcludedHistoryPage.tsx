...
import { excludedEntriesApi } from '@/web/shared/lib/api/backend.api';
...
export default function ExcludedHistoryPage() {
  ...
  const loadData = async () => {
    ...
    // excludedEntriesApi returns excluded journal entries with joined document/client data
    const clientId = currentWorkflow?.clientId;
    if (!clientId) {
      // Without a client filter, we cannot call excludedEntriesApi (requires client_id)
      setEntries([]);
      setLoading(false);
      return;
    }

    const { data, error } = await excludedEntriesApi.getByClient(clientId);
