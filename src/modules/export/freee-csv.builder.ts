/**
 * @module freee 取込 CSV 生成
 * @description freee の仕訳インポート用21列 CSV を生成する純粋関数。
 */
...
/**
 * FreeeCsvRow 配列からヘッダー付き CSV 文字列を生成する。
 * 摘要はダブルクォートでエスケープ。
 */
export function buildFreeeCsv(rows: FreeeCsvRow[]): string {
  ...
  const lines = rows.map(r =>
    [
      r.entryType, r.date, r.closingType,
      r.debitAccount, r.debitSubAccount, r.debitPartner, r.debitDepartment, r.debitTaxCategory, r.debitAmount,
      r.creditAccount, r.creditSubAccount, r.creditPartner, r.creditDepartment, r.creditTaxCategory, r.creditAmount,
      r.description, r.tag1, r.tag2, r.tag3, r.adjustment, r.segment,
