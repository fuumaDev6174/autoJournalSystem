...
export default function ClientListPage() {
  ...
  const loadData = async () => {
    ...
    // 業種情報
    const { data: ind } = await supabase.from('industries').select('*').eq('id', industryId).single();
    ...
    const { data: indirectClients } = await supabase
    ...
    for (const ci of (indirectClients || [])) {
      const c = Array.isArray(ci.clients) ? ci.clients[0] : ci.clients;
    ...
    const enriched = await Promise.all(allClients.map(async (c) => {
      const { count } = await supabase
        ...
        .eq('client_id', c.id)
        .eq('is_active', true);
      return { ...c, clientRuleCount: count || 0 };
    }));
