...
import ImageViewer from '@/client/components/review/ImageViewer';
import EntryCard from '@/client/components/review/EntryCard';
import MultiEntryPanel from '@/client/components/review/MultiEntryPanel';
...
export default function ReviewPage() {
  ...
  const loadAllData = async () => {
    ...
    const { data: clientIndustryData } = await supabase.from('client_industries').select('industry_id').eq('client_id', clientId);
    const { data: clientRow } = await supabase.from('clients').select('industry_id').eq('id', clientId).single();
    ...
    if (clientIndustryIds.length > 0) {
      const { data: indAccounts } = await supabase
        .from('account_items').select('id, name, industry_id, tax_category_id')
        .in('industry_id', clientIndustryIds).eq('is_active', true);
        ...
        if (currentUser) {
          if (isManagerOrAdmin) {
            await supabase.from('journal_entry_approvals').insert([{
              journal_entry_id: entryId, approver_id: currentUser.id,
              approval_status: 'approved', approval_level: 1,
              approved_at: new Date().toISOString(), comments: '自己確認・承認',
            }]);
          } else {
            await supabase.from('journal_entry_approvals').insert([{
              journal_entry_id: entryId, approver_id: currentUser.id,
    ...
    if (form.supplierId && item.supplierName) {
      ...
      if (matchedSupplier && normalizeJapanese(matchedSupplier.name) !== normalizeJapanese(item.supplierName)) {
        const { data: existingAlias } = await supabase
          .from('supplier_aliases').select('id')
          .eq('supplier_id', form.supplierId).eq('alias_name', item.supplierName).limit(1);
        if (!existingAlias || existingAlias.length === 0) {
          await supabase.from('supplier_aliases').insert({ supplier_id: form.supplierId, alias_name: item.supplierName, source: 'ai_suggested' });
        }
      }
    }
        ...
        if (cd?.organization_id && authUser) {
          ...
          if (corrections.find(c => c.field_name === 'account_item_id')) {
            const { data: samePattern, count } = await supabase
              .from('journal_entry_corrections').select('id', { count: 'exact' })
              .eq('client_id', currentWorkflow.clientId).eq('field_name', 'account_item_id')
              .eq('corrected_value', form.accountItemId!).eq('supplier_name', item.supplierName || '').eq('rule_suggested', false);
            if ((count || 0) >= 3) {
              setAddRule(true);
              setRuleSuggestion(`同じ修正が${count}回検出されました。ルール追加を推奨します。`);
              if (samePattern) {
                await supabase.from('journal_entry_corrections').update({ rule_suggested: true })
                  .eq('client_id', currentWorkflow.clientId).eq('field_name', 'account_item_id')
                  .eq('corrected_value', form.accountItemId!).eq('supplier_name', item.supplierName || '');
              }
            }
          }
        }
  ...
  const handleApproveFromList = async (entryId: string) => {
    ...
    await supabase.from('journal_entry_approvals').insert([{
      journal_entry_id: entryId, approver_id: currentUser.id,
      approval_status: 'approved', approval_level: 1,
      approved_at: new Date().toISOString(), comments: '一覧画面から承認',
    }]);
    await loadAllData();
  };
  ...
  const handleBeforeNext = async (): Promise<boolean> => {
    ...
    if (allIds.length > 0) await supabase.from('journal_entries').update({ status: 'posted' }).in('id', allIds);
          ...
          {viewMode === 'list' && isManagerOrAdmin && reviewedCount > 0 && (
            <button
              onClick={async () => {
                ...
                await supabase.from('journal_entries').update({ status: 'approved' }).in('id', reviewedEntries.map(e => e.id));
                const approvals = reviewedEntries.map(e => ({
                  journal_entry_id: e.id, approver_id: currentUser.id,
                  approval_status: 'approved' as const, approval_level: 1,
                  approved_at: new Date().toISOString(), comments: '一括承認',
                }));
