// 証憑分類プロンプト — Gemini に送る書類分類プロンプト（全63書類種別を網羅）
...
■ 絶対に守るルール
━━━━━━━━━━━━━━━━━━━━━━━
1. JSONオブジェクトのみを出力すること
2. \`\`\`json 等のマークダウンコードブロック記法は絶対に使わないこと
3. 前置きテキスト、説明文、改行も不要。最初の文字が { で始まり、最後の文字が } で終わること
4. document_type_code は上記リストのコードをそのまま使うこと（造語禁止）
5. 判断に迷う場合は confidence を正直に下げること（0.8未満なら後続処理で再確認される）
6. other_journal は「仕訳が必要そうだが種別が特定できない」ときのみ使用
7. other_ref は「仕訳不要かつ種別不明」のときのみ使用`;